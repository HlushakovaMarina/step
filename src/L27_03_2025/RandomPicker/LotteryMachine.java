...
public class LotteryMachine<T extends Participant> {
    ...
    public void add(T item) {
        ...
        if (item.getAge() < 18) {
            System.out.println("Участник " + item.getName() + " младше 18 лет и не может участвовать.");
            return;
        }
        boolean uniqueParticipant = allItems.stream().anyMatch
                (existingItem -> existingItem.getPassportId().
                        equals(item.getPassportId()));
        if (uniqueParticipant) {
            System.out.println("Участник с passportId " + item.getPassportId() + " уже добавлен.");
            return;
        }
        allItems.add(item);
    }
